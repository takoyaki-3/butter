...
const internal = {
  async setCA(rootCA) {
    ...
    try {
      ...
      try {
        // CONFIG.rootCAへのアクセスが失敗またはJSON形式でない場合、代替のURLにアクセスを試みる
        req = await this.fetchAndParseJSON("https://sub-butter.takoyaki3.com/v0.0.0/root.json");
      } catch {
        throw new Error("Failed to fetch and parse both primary and fallback rootCA"); // 両方のURLへのアクセスが失敗した場合のエラーメッセージ
      }
    }
