  ...
  async setCA(rootCA) {
    ...
    try {
      ...
      for (let i = 0; i < RUNTIME.CA.hosts.length; i++) {
        ...
        const data = await helper.fetchJSON(`${host}/datalist.json`)
        ...
        console.log(updatedISO, updatedDate)
...
const Butter = {
  ...
  async getHostUpdated() {
    ...
    for (let i = 0; i < RUNTIME.CA.hosts.length; i++) {
      ...
      if (data.updated == null) continue
      const updatedISO = data.updated.replace(/_/g, ':')
      const updated = new Date(updatedISO) // Dateオブジェクトを生成します
  ...
  init(butterRoot = "https://butter.takoyaki3.com/v0.0.0/root.json") {
    internal.setCA(butterRoot)
  }
};
