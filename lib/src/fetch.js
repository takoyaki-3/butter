...
const internal = {
  ...
  getFactory(target){
    return async (gtfsID, versionID = 'optional') => {
      ...
      const versionID = await Butter.getVersionId(gtfsID, versionID);
      const data = await helper.fetchCSV(`${RUNTIME.host}/${gtfsID}/${versionID}/${urls[target]}`, RUNTIME.pub_key)
      return data
    }
  },
}
...
const Butter = {
  ...
  async init (butterRoot = 'https://butter.takoyaki3.com/v0.0.0/root.json') {
    ...
    for (const key in internal.urls) {
      Butter[key] = internal.getFactory(key)
    }
  },
