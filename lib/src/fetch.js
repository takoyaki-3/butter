...
const Butter = {
  ...
  async getVehiclePositionFromURL (url) {
    ...
    try {
      // GTFS-RTデータの取得
      const data = await helper.fetchAsArrayBuffer('https://cros-proxy.api.takoyaki3.com/' + url)
   ...
   * 初期化処理を行います。
   */
  async init (butterRoot = 'https://butter.takoyaki3.com/v0.0.0/root.json', config = { useFetch: false }) {
