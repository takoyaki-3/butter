...
(function () {
    ...
    const Butter = {
        ...
        utils: {
            ...
            async fetchTimeTable(gtfsID, options, version = "optional") {
                const optionSample = {
                    // INDEX1
                ...
                if (options.stop_ids) {
                    ...
                    const data = await Promise.all([
                        Butter.utils.findTimeTableByStopID(gtfsID, version, stopID, options.date),
                        Butter.utils.findTrips(gtfsID, version, constraints)
                    ])
                    ...
                    const data = await Butter.utils.findTimeTableByTripIDs(gtfsID, version, options.trip_ids)
                    for (const e of data) {
                        trips.push({
                            trip_id: e.trip_id,
                            stop_id: e.stop_id,
                ...
                if (options.start_time) {
                    trips = trips.filter(e => e.date >= options.start_time)
                }
                if (options.end_time) {
                    trips = trips.filter(e => e.date <= options.end_time)
                }
