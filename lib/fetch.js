    ...
    const Butter = {
        ...
        utils: {
            ...
            async findTrips(gtfsID, versionID, stopIDs) {
                const data = await Promise.all(stopIDs.map(e => Butter.utils.getTimeTableByStopID(gtfsID, versionID, e)))
            ...
            async findTimeTableByStopID(gtfsID, versionID, stopID, date) {
                const data = await Promise.all([
                    Butter.utils.getTimeTableByStopID(gtfsID, versionID, stopID),
                    Butter.utils.getServiceIDs(gtfsID, versionID, date)
                ])
            ...
            async findTimeTableByTripIDs(gtfsID, versionID, TripIDs) {
                const data = await Promise.all(TripIDs.map(e => Butter.utils.getTimeTableByTripID(gtfsID, versionID, e)))
                let ret = []
                for (const e of data) {
                    ret = ret.concat(e)
                }
            ...
            async fetchTimeTable(gtfsID, versionID, options) {
                const optionSample = {
                    // 必須
                    "date": "20230407",
                    // INDEX1
                ...
                if (options.stop_ids) {
                    ...
                    const data = await Promise.all([
                        Butter.utils.findTimeTableByStopID(gtfsID, versionID, stopID, options.date),
                        Butter.utils.findTrips(gtfsID, versionID, constraints)
                    ])
                    ...
                    for (const e of filtered) {
                        if (!trips[e.trip_id]) {
                            trips[e.trip_id] = {
                                stop_times: [],
                                realtime_info: "NOT IMPLEMENTED",
                            }
                        }
                        trips[e.trip_id].stop_times.push({
                            stop_id: e.stop_id,
                    ...
                    const data = await Butter.utils.findTimeTableByTripIDs(gtfsID, versionID, options.trip_ids)
                    for (const e of data) {
                        if (!trips[e.trip_id]) {
                            trips[e.trip_id] = {
                                stop_times: [],
                ...
                for (const key in trips) {
                    if (options.start_time) {
                        trips[key].stop_times = trips[key].stop_times.filter(e => e.date >= options.start_time)
                    }
                    if (options.end_time) {
                        trips[key].stop_times = trips[key].stop_times.filter(e => e.date <= options.end_time)
                    }
                    trips[key].stop_times = trips[key].stop_times.sort((a, b) => (a.date < b.date) ? -1 : 1)
                }
                return {
